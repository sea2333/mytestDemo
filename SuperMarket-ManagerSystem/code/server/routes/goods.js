...
router.all('*', (_req, res, next) => {
 ...
 router.post('/addgoods', (req, res) => {
	 ...
	 let sql = `INSERT INTO goods(theclass,code,name,saleprice,price,originprice,number,weight,unit,preferential,promote,introduce,supplierId,factoryId) 
	 VALUES('${ theclass }','${ code }','${ name }','${ saleprice }','${ price }','${ originprice }','${ number }','${ weight }','${ unit }','${ preferential }','${ promote }','${ introduce }','${ supplierId }','${ factoryId }')`;
 ...
 router.get('/delgoods', (req, res) => {
	 let { id } = req.query
	 console.log(id)
     let sql = `delete from goods where code='${ id }'`
     connection.query(sql, (err, data) => {
         if(err) throw err;
         if(data.affectedRows > 0){
 ...
 router.get('/returndata', (req, res) => {
 	let { code } = req.query;
 	let sql = `select * from goods where code = '${ code }'`;
 	connection.query(sql, (err, data) => {
 		if (err) throw err;
 		if(data.length != 0){
 		    res.send(data)
 ...
 router.post('/updategoods',(req, res) => {
 	let {
          ...
          number,
          code} = req.body;
 	let sql = `update goods set name='${ name }', theclass='${ theclass }', price='${ price }', saleprice='${ saleprice }', number='${ number }' where code=${ code }`;
 	connection.query(sql, (err, data) => {
		 console.log(data)
 		if(data){
